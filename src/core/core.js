...
function routeRender(routes) {
    //접속할 때 해시 모드가 아니면(해시가 없으면) 리다이렉트
    if(!location.hash) {
        history.replaceState(null, '', '/#/') //(상태, title, 주소)
    }
    const routeView = document.querySelector('router-view')
    const [hash, queryString = ''] = location.hash.split('?') 
    //location.hash는 (있으면)쿼리스트링까지 같이 연결된 문자열을 반환하므로 

    // 1) 쿼리스트링을 객체로 변환해(1-1) 히스토리 객체에 저장하기(1-2)
...
export function createRouter(routes) {
    ...
    return function() {
        ...
        routeRender(routes) //최초 한번은 페이지 이동이 없으므로 직접 호출해주기
    }
}
